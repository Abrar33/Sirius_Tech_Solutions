...
const servicesData: ServiceItem[] = [
  ...
  },
  {
    ...
    description: "Scale your team instantly with 500+ certified IT experts across leading technologies. From front-end and back-end developers to mobile engineers, UI/UX designers, DevOps specialists, testers, and project managers, we provide the right talent, right when you need it.",
  ...
  },
  {
    ...
    description: "With 200+ successful projects delivered, we specialize in custom-built software that solves complex business needs.",
    points: [
  ...
  },
  {
    ...
    description: "We deliver 24/7 end-to-end support to keep your business running smoothly. With a 99% resolution rate and fast response times, our multi-level support ensures no issue goes unresolved. From quick fixes to complex escalations, we ensure seamless operations and zero downtime.",
  ...
  return (
    <div className="bg-black min-h-screen font-sans selection:bg-teal-500/30">
      {servicesData.map((service) => (
        <section key={service.id} className="py-20 px-6 border-b border-gray-900">
          <div className={`max-w-7xl mx-auto flex flex-col ${service.reverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-16 items-start`}>
            ...
            >
              ...
              {service.type === "grid" && service.subheading && (
                <h3 className="text-xl font-bold text-white pt-8">{service.subheading}</h3>
              )}
              
              {service.type === "grid" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 pt-4">
                  {service.points.map((point, i) => <ListItem key={i} text={point} />)}
                </div>
              )}
            ...
            >
              {service.type === "support" ? (
                <div className="space-y-6">
                  {service.levels.map((level, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row items-start sm:items-center gap-6 group">
                      <div className="bg-[#111] border border-gray-800 rounded-lg px-6 py-4 min-w-[180px] text-center shadow-xl group-hover:border-teal-500/50 transition-colors">
                ...
                <div className="relative group">
                  <div className="absolute -inset-1 bg-gradient-to-r from-teal-500/20 to-blue-500/20 rounded-[2rem] blur opacity-25 group-hover:opacity-50 transition duration-1000"></div>
                  <img 
                    src={service.image} 
                    alt={service.title} 
                    className="relative rounded-[2rem] w-full h-auto object-cover border border-gray-800 shadow-2xl"
                  />
                </div>
              )}
            </motion.div>
